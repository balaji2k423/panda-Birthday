...
const CARDS: Card[] = [
  ...
  },
  {
    ...
    sub: "where the best convos happen",
    shimmer: "#c084fc",
    circle: "#a855f7",
    type: "chat",
    bg: "linear-gradient(160deg,hsl(270 50% 12%) 0%,hsl(300 45% 10%) 100%)",
    messages: [
      { from: "them", text: "ok but why is today so long 😭", time: "11:42" },
      { from: "me",   text: "ikr i've checked the clock 400 times", time: "11:43" },
      { from: "them", text: "at least we have plans after 🥲", time: "11:44" },
      { from: "me",   text: "that's the only thing keeping me alive rn", time: "11:44", react: "😭" },
    ],
  },
  {
    tag: "calls",
            ...
            {[{ label: "←", delta: -1 }, { label: "→", delta: 1 }].map(({ label, delta }) => (
              <button key={label} onClick={() => goTo(cur + delta)}
                ...
                style={{ width: 36, height: 36, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.1)",
                  ...
                  transition: "all 0.2s", opacity: (delta === -1 ? cur === 0 : cur === CARDS.length - 1) ? 0.2 : 1,
                }}>{label}</button>
            ))}
          </div>
        </motion.div>
